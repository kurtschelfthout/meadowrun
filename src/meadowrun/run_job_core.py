    ...
    async def run_job(self, job: Job) -> JobCompletion[Any]:
        with fabric.Connection(
            ...
            try:
                ...
                with eliot.start_action(action_type="wait_for_ssh_connection"):
                    home_result = await _retry(
                        lambda: connection.run(
                            "echo $HOME", hide=True, in_stream=False
                        ),
                ...
                with eliot.start_action(action_type="remote mkdir"):
                ...
                with eliot.start_action(action_type="serialize and send job"):
                ...
                with eliot.start_action(action_type="run job"):
                    ...
                    def run_and_wait() -> None:
                        try:
                            ...
                            task_id = (
                                eliot.current_action().serialize_task_id().decode()
                            )
                            addendum = f" --eliot-task-id {task_id}"
                            returned_result = connection.run(
                                command + addendum, in_stream=False
                            )
                    ...
                    threading.Thread(
                        target=eliot.preserve_context(run_and_wait)
                    ).start()
                    result = await result_future

                # TODO consider using result.tail, result.stdout

                with eliot.start_action(action_type="get result"):
                    ...
                    if process_state.state == ProcessState.ProcessStateEnum.SUCCEEDED:
                        ...
                        if (
                            job_spec_type == "py_function"
                            or process_state.pickled_result
                        ):
                ...
                if job_io_prefix:
                    with eliot.start_action(action_type="cleanup"):
                        remote_paths = " ".join(
                            [
